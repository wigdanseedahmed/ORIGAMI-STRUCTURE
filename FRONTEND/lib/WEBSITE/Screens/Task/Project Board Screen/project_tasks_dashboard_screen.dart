...
class ProjectTasksDashboardScreenWS extends StatefulWidget {
  final String projectName;

  const ProjectTasksDashboardScreenWS({Key? key, required this.projectName}) : super(key: key);

  @override
  State<ProjectTasksDashboardScreenWS> createState() =>
      _ProjectTasksDashboardScreenWSState();
}

class _ProjectTasksDashboardScreenWSState
    extends State<ProjectTasksDashboardScreenWS> {
  ...
  Future<UserModel> readingUserJsonData() async {
    ...
    if (response.statusCode == 200) {
      ...
      userTaskChartData = [
        AllUserTaskChartDataTasksDashboardWS(
  ...
  late List<AllUserTaskChartDataTasksDashboardWS> userTaskChartData = <AllUserTaskChartDataTasksDashboardWS>[];
  ...
  Future<List<TaskModel>> readingTasksJsonData() async {
    ...
    if (response.statusCode == 200) {
      readTaskJsonFileContent = taskModelFromJson(response.body).where((element) => element.projectName == widget.projectName).toList();
      ...
      children: [
        ...
        SfCircularChart(
          ...
          series: <CircularSeries>[
            DoughnutSeries<AllUserTaskChartDataTasksDashboardWS, String>(
              dataSource: userTaskChartData,
              pointColorMapper: (AllUserTaskChartDataTasksDashboardWS data, _) => data.color,
              xValueMapper: (AllUserTaskChartDataTasksDashboardWS data, _) => data.x,
              yValueMapper: (AllUserTaskChartDataTasksDashboardWS data, _) => data.y,
...
class AllUserTaskChartDataTasksDashboardWS {
  AllUserTaskChartDataTasksDashboardWS(this.x, this.y, [this.color]);
