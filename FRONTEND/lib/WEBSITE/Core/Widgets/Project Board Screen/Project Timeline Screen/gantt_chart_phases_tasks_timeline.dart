...
class PhasesTasksGanttChartWS extends StatelessWidget {
  ...
  int viewRangeToFitScreen = 8;
  Animation<double>? width;

  PhasesTasksGanttChartWS({
    Key? key,
  ...
  Widget buildHeader(Size screenSize, Color color) {
    ...
    for (int i = 0; i < projectDurationInWeekRange!; i++) {
      headerItems.add(
        SizedBox(
          width: screenSize.width * 0.008 * 7,
          child: Align(
    ...
    gridColumns.add(
      Padding(
        padding: EdgeInsets.only(
          left: (screenSize.width / viewRangeToFitScreen) + calculateDistanceToLeftBorder(DateTime.now()) * screenSize.width * 0.008,
        ) ,
        child: Container(
          decoration: BoxDecoration(
            border: Border(
  ...
  List<Widget> buildChartBars(
    ...
    for (int i = 0; i < tasksData.length; i++) {
      ...
      if (remainingWidth > 0) {
        chartBars.add(
          Container(
            width: (calculateDistanceToLeftBorder(DateTime.parse(tasksData[i].startDate!)) * screenSize.width * 0.008) +
                ((DateTime.parse(tasksData[i].deadlineDate!).difference(DateTime.parse(tasksData[i].startDate!)).inDays).toDouble()) * screenSize.width * 0.008,
            alignment: Alignment.centerLeft,
            child: Padding(
              padding: EdgeInsets.only(
                  bottom: screenSize.height * 0.008,
                  left: (calculateDistanceToLeftBorder(DateTime.parse(tasksData[i].startDate!)) * screenSize.width * 0.008)),
              child: LinearPercentIndicator(
                width: ((DateTime.parse(tasksData[i].deadlineDate!).difference(DateTime.parse(tasksData[i].startDate!)).inDays).toDouble()) * screenSize.width * 0.008,
                animation: true,
                animationDuration: 1000,
                lineHeight: screenSize.height * 0.035,
  ...
  Widget buildChartForEachPhase(
    ...
    return SizedBox(
      height: chartBars.length * screenSize.height * 0.0458 +
                  ...
                  children: <Widget>[
                    Row(
                      children: <Widget>[
                        Container(
                          width: screenSize.width / viewRangeToFitScreen,
                          height: chartBars.length * screenSize.height * 0.0458 + screenSize.height * 0.0042,
                          color: color.withAlpha(100),
                          child: Center(
                            child: Text(
                              phase.phase!,
                        ...
                        Padding(
                          padding:
                          EdgeInsets.only(top: screenSize.height * 0.0012),
                          child: Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: chartBars,
                          ),
