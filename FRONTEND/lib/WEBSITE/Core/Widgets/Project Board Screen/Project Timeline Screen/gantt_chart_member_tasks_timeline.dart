...
class MembersTasksGanttChartWS extends StatelessWidget {
  final AnimationController? animationController;
  final DateTime? projectStartDate;
  final DateTime? projectEndDate;
  final List<TaskModel>? projectTaskData;
  final List<MembersModel>? membersOfProject;
  ...
  MembersTasksGanttChartWS({
    Key? key,
    this.animationController,
    this.projectStartDate,
    this.projectEndDate,
    this.projectTaskData,
    this.membersOfProject,
  }) : super(key: key) {
  ...
  Color randomColorGenerator() {
    var r = Random();
    return Color.fromRGBO(r.nextInt(106), r.nextInt(106), r.nextInt(106), 0.70);
  }
      ...
      Padding(
        padding: EdgeInsets.only(
          left: (screenSize.width / viewRangeToFitScreen) +
              calculateDistanceToLeftBorder(DateTime.now()) * screenSize.width * 0.008,
        ),
        child: Container(
          decoration: BoxDecoration(
            border: Border(
  ...
  Widget buildChartForEachPhase(
      List<TaskModel> taskData, Size screenSize, MembersModel member) {
        ...
        children: <Widget>[
          Stack(
            ...
            children: <Widget>[
              ...
              Container(
                margin: EdgeInsets.only(top: screenSize.height * 0.095),
                child: Column(
                  children: <Widget>[
                    Row(
                      children: <Widget>[
                        Container(
                          width: screenSize.width / viewRangeToFitScreen,
                          height:
                          chartBars.length * screenSize.height * 0.0458 +
  ...
  List<Widget> buildChartContent(Size screenSize) {
    ...
    membersOfProject!.forEach((member) {
      List<TaskModel> tasksOfPhase = [];

      tasksOfPhase = projectTaskData!
          .where((task) => task.assignedTo!.contains(member.memberUsername))
          .toList();

      if (tasksOfPhase.isNotEmpty) {
        chartContent
            .add(buildChartForEachPhase(tasksOfPhase, screenSize, member));
      }
    });
