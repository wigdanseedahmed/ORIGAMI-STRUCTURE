...
class FutureProjectScreenMA extends StatefulWidget {
  static const String id = 'future_project_screen';

  final String username;

  const FutureProjectScreenMA({Key? key, required this.username}) : super(key: key);
  @override
  _FutureProjectScreenMAState createState() => _FutureProjectScreenMAState();
}

class _FutureProjectScreenMAState extends State<FutureProjectScreenMA>
    with TickerProviderStateMixin {
  late List<ProjectModel> _futureProjects = <ProjectModel>[];

  Future<List<ProjectModel>> readingFutureProjectJsonData() async {
    ...
    if (response.statusCode == 200) {
      _futureProjects = projectModelFromJson(response.body).where((element) => element.members!.any((element) =>
      element.memberUsername == widget.username))
          .toList();
      // print("ALL  projects: $_allProjects");

      return _futureProjects;
    } else {
    ...
    return Scaffold(
      body: SafeArea(
        child: FutureBuilder<List<ProjectModel>>(
          future: readingFutureProjectJsonData(),
                    ...
                    return FutureBuilder(
                      ...
                      builder: (context, data) {
                        if (data.hasError) {
                          ...
                          return _buildFutureProjectList();
                        } else {
                          return const Center(
  ...
  Widget _buildFutureProjectList() {
    var futureProjectFilter = _futureProjects.where((element) =>
    element.status == 'Future',);

    return Scaffold(
      body: (futureProjectFilter.isEmpty)
          ? const Text(
        "No projects found!",
        textAlign: TextAlign.center,
        style: TextStyle(color: Colors.black, fontSize: 16,),)
          : ListView(
        children: futureProjectFilter
