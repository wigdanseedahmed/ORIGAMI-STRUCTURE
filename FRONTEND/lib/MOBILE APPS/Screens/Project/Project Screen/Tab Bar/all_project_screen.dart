...
class AllProjectScreenMA extends StatefulWidget {
  static const String id = 'all_project_screen';

  final String username;

  const AllProjectScreenMA({Key? key, required this.username}) : super(key: key);

  @override
  _AllProjectScreenMAState createState() => _AllProjectScreenMAState();
}

class _AllProjectScreenMAState extends State<AllProjectScreenMA>
  ...
  Future<List<ProjectModel>> readingAllProjectJsonData() async {
    ...
    if (response.statusCode == 200) {
      _allProjects = projectModelFromJson(response.body).where((element) => element.members!.any((element) =>
  ...
  Widget _buildAllProjectList() {
    var allProjectFilter = _allProjects;
   /* var allProjectFilter = _allProjects.where((element) =>
        element.status == 'Open' ||
