...
class ClosedProjectScreenMA extends StatefulWidget {
  static const String id = 'closed_project_screen';

  final String username;

  const ClosedProjectScreenMA({Key? key, required this.username}) : super(key: key);
  @override
  _ClosedProjectScreenMAState createState() => _ClosedProjectScreenMAState();
}

class _ClosedProjectScreenMAState extends State<ClosedProjectScreenMA>
    with TickerProviderStateMixin {
  late List<ProjectModel> _closedProjects = <ProjectModel>[];

  Future<List<ProjectModel>> readingClosedProjectJsonData() async {
    ...
    if (response.statusCode == 200) {
      _closedProjects = projectModelFromJson(response.body).where((element) => element.members!.any((element) =>
      element.memberUsername == widget.username))
          .toList();
      // print("ALL  projects: $_allProjects");

      return _closedProjects;
    } else {
    ...
    return Scaffold(
      body: SafeArea(
        child: FutureBuilder<List<ProjectModel>>(
          future: readingClosedProjectJsonData(),
                    ...
                    return FutureBuilder(
                      ...
                      builder: (context, data) {
                        if (data.hasError) {
                          ...
                          return _buildClosedProjectList();
                        } else {
                          return const Center(
  ...
  Widget _buildClosedProjectList() {
    var closedProjectFilter = _closedProjects.where((element) =>
    element.status == 'Closed',);

    return Scaffold(
      body: (closedProjectFilter.isEmpty)
          ? const Text(
        "No projects found!",
        textAlign: TextAlign.center,
        style: TextStyle(color: Colors.black, fontSize: 16,),)
          : ListView(
        children: closedProjectFilter
