...
class ExpiredProjectScreenMA extends StatefulWidget {
  static const String id = 'expired_project_screen';

  final String username;

  const ExpiredProjectScreenMA({Key? key, required this.username}) : super(key: key);
  @override
  _ExpiredProjectScreenMAState createState() => _ExpiredProjectScreenMAState();
}

class _ExpiredProjectScreenMAState extends State<ExpiredProjectScreenMA>
    with TickerProviderStateMixin {
  late List<ProjectModel> _expiredProjects = <ProjectModel>[];

  Future<List<ProjectModel>> readingExpiredProjectJsonData() async {
    ...
    if (response.statusCode == 200) {
      _expiredProjects = projectModelFromJson(response.body).where((element) => element.members!.any((element) =>
      element.memberUsername == widget.username))
          .toList();
      // print("ALL  projects: $_allProjects");

      return _expiredProjects;
    } else {
    ...
    return Scaffold(
      body: SafeArea(
        child: FutureBuilder<List<ProjectModel>>(
          future: readingExpiredProjectJsonData(),
                    ...
                    return FutureBuilder(
                      ...
                      builder: (context, data) {
                        if (data.hasError) {
                          ...
                          return _buildExpiredProjectList();
                        } else {
                          return const Center(
  ...
  Widget _buildExpiredProjectList() {
    var expiredProjectFilter = _expiredProjects.where((element) =>
    element.status == 'Expired',);

    return Scaffold(
      body: (expiredProjectFilter.isEmpty)
          ? const Text(
        "No projects found!",
        textAlign: TextAlign.center,
        style: TextStyle(color: Colors.black, fontSize: 16,),)
          : ListView(
        children: expiredProjectFilter
