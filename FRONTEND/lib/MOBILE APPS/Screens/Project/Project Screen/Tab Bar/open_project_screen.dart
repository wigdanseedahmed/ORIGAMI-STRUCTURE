...
class OpenProjectScreenWS extends StatefulWidget {
  static const String id = 'open_project_screen';

  final String username;

  const OpenProjectScreenWS({Key? key, required this.username}) : super(key: key);
  @override
  _OpenProjectScreenWSState createState() => _OpenProjectScreenWSState();
}

class _OpenProjectScreenWSState extends State<OpenProjectScreenWS>
    with TickerProviderStateMixin {
  late List<ProjectModel> _openProjects = <ProjectModel>[];

  Future<List<ProjectModel>> readingOpenProjectJsonData() async {
    ...
    if (response.statusCode == 200) {
      _openProjects = projectModelFromJson(response.body).where((element) => element.members!.any((element) =>
    ...
    return Scaffold(
      body: SafeArea(
        child: FutureBuilder<List<ProjectModel>>(
          future: readingOpenProjectJsonData(),
  ...
  Widget _buildOpenProjectList() {
    var openProjectFilter = _openProjects.where((element) =>
    element.status == 'Open',);

    return Scaffold(
      body: (openProjectFilter.isEmpty)
          ? const Text(
        "No projects found!",
        textAlign: TextAlign.center,
        style: TextStyle(color: Colors.black, fontSize: 16,),)
          : ListView(
        children: openProjectFilter
